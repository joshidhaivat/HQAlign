...
def get_reads_from_fastq(filename):
    ...
    seq_indicator=0
    for aline in fid:
        if(aline[0]=='@' and seq_indicator==0):
            seq_indicator=1
            name.append(aline)
            continue
        if(seq_indicator==1):
...
def get_genomeDict_from_fasta(filename):
    genome = []
    num_genome = 0
    filename = str(filename)
    fid = open(filename,'r')
...
def print_refDict_to_fasta(ref,filename):
    filename = str(filename)
    fid = open(filename,'w')
    for aname in ref.keys():
        fid.write('>{}\n'.format(aname))
        fid.write('{}\n\n'.format(ref[aname]))
    fid.close()
...
def extract_from_paf_v1(filename,reads_dict={},ref_dict={},cigar_string=False):
    start_time = time.time()
    fid = open(filename,'r')
    out = []
    out_ed = []
        ...
        for ii in range(12,len(cols)):
            ...
            if cigar_string==True and len(cols[ii])>5 and cols[ii][:5]=='cg:Z:':
                ...
                if map_type[5] == 'P':
                    ...
                    pass#print('Unknown mapping type: {}\n'.format(cols[ii]))
            elif len(cols[ii])>5 and cols[ii][:5] == 'cm:i:':
                temp[13] = int(cols[ii][5:]) # Num of anchors
            elif len(cols[ii])>5 and cols[ii][:5] == 's1:i:':
                temp[14] = int(cols[ii][5:]) # Chaining score of Primary alignment
...
#                     break
                    ...
                    try:
                        ...
                        continue
                elif len(element)>7 and element[:7] == 'RNAMES=':
                    try:
                        rnames = element[7:]
                        supporting_reads = rnames.split(',')
                        continue
                    except:
                        continue
            out.append([index,precise,ref_start,ref_end,sv_length,read_support,ref_chr,sv_type])
            out_seq.append(seq)
            supporting_read_names.append(supporting_reads)
    fid.close()
    if len(supporting_read_names)==0:
        return out,out_seq
    else:
        return out,out_seq,supporting_read_names