...
kseq_t *rc_seq;
char *rc_read_name[100000];
char *rc_read_seq[100000];
int line_count = 0;

int main(int argc, char *argv[])
{
	const char *opt_str = "2aSDw:k:K:t:r:f:Vv:g:G:I:d:XT:s:x:Hcp:M:n:z:A:B:O:E:m:N:Qu:R:hF:LC:yYPo:e:U:q:";
	...
	gzFile fp;
	int l;
	//filename = "/home/djjoshi/SV_call/simulations/notebooks/test_dbg_alignments/test_rc_read_20285.fasta";
...
//		fprintf(fp_help, "    -v INT       verbose level [%d]\n", mm_verbose);
	...
	rc_seq = kseq_init(fp);
	// fprintf(stderr,"\nReading the reverse file now\n");
	int max_name_len = 0;
	int max_seq_len = 0;
	while ((l = kseq_read(rc_seq)) >= 0){
		if (strlen(rc_seq->name.s) > max_name_len){
			max_name_len = strlen(rc_seq->name.s) + 1;
		}
		if (strlen(rc_seq->seq.s) > max_seq_len){
			max_seq_len = strlen(rc_seq->seq.s) + 2;
		}		
		line_count += 1;
		//fprintf(stderr, "\n%p/%p -> qname: %s\n%s", &rc_seq->name.s, &rc_seq->seq.s, rc_seq->name.s, rc_seq->seq.s);
	}
	// fprintf(stderr, "\nmax name len = %ld, max seq len = %ld", max_name_len, max_seq_len);
	gzclose(fp);
	fp = gzopen(filename, "r");	
	rc_seq = kseq_init(fp);
	i = 0;
	while ((l = kseq_read(rc_seq)) >= 0){
		if (i == 0){
			rc_read_name[0] = (char*)malloc(sizeof(char)*max_name_len*100000);
			rc_read_seq[0] = (char*)malloc(sizeof(char)*max_seq_len*100000);
		} else{
			rc_read_name[i] = rc_read_name[i-1] + max_name_len;
			rc_read_seq[i] = rc_read_seq[i-1] + max_seq_len;
		}
		//fprintf(stderr,"\nA) Successful run line %d, strlen = %d", i, strlen(rc_seq->name.s));
		strcpy(rc_read_name[i],rc_seq->name.s);
		//fprintf(stderr,"\nB) Successful run line %d, strlen = %d\n", i, strlen(rc_seq->seq.s));
		strcpy(rc_read_seq[i], rc_seq->seq.s);
		i += 1;
	}
	gzclose(fp);
	/*
	for (i=0; i<line_count; ++i){
		fprintf(stderr,"\nqname:%ld; length:%ld",strlen(rc_read_name[i]), strlen(rc_read_seq[i]));
	}
	fprintf(stderr, "\nLoaded FN: %s; qname: %s; line count=%ld\n", filename, rc_seq->name.s, line_count);
	*/
