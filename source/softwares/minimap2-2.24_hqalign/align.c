...
static int mm_test_zdrop(void *km, const mm_mapopt_t *opt, const uint8_t *qseq, const uint8_t *tseq, uint32_t n_cigar, uint32_t *cigar, const int8_t *mat, const uint8_t *rc_qseq, int read_length)
{
	uint32_t k;
	...
	if (!(opt->flag&(MM_F_SPLICE|MM_F_SR|MM_F_FOR_ONLY|MM_F_REV_ONLY)) && max_zdrop > opt->zdrop_inv && q_len < opt->max_gap && t_len < opt->max_gap) {
		...
		void *qp;
		int q_off, t_off;
		//fprintf(stderr, "\nTesting the inversion in z-drop: qlen = %d; qseq:%p/%p--->diff:%ld: qs = %d, qe = %d", q_len, &qseq[pos[1][0]],qseq, (long int)&qseq[pos[1][0]] - (long int)qseq, pos[1][0], pos[1][1]);
		qseq2 = (uint8_t*)kmalloc(km, q_len);
		for (i = 0; i < q_len; ++i) {
			//int c = qseq[pos[1][1] - i - 1];
			//int temp = c >= 4? 4 : 3 - c;
			//qseq2[i] = c >= 4? 4 : 3 - c;
			int temp0 = rc_qseq[i-pos[1][1]+1];
			qseq2[i] = temp0 >= 4? 4 : temp0;
			//if (qseq2[i] != temp) fprintf(stderr, "\ni = %d: temp/qseq2[i] = %d/%d; index: c/qseq2[i] = %d/%d", i, temp, qseq2[i], pos[1][1]-i-1, read_length - pos[1][1] + i);
		}
...
static void mm_align1(void *km, const mm_mapopt_t *opt, const mm_idx_t *mi, int qlen, uint8_t *qseq0[2], mm_reg1_t *r, mm_reg1_t *r2, int n_a, mm128_t *a, ksw_extz_t *ez, int splice_flag)
{
	...
	uint8_t *tseq, *qseq, *junc, *rc_qseq;
	...
	for (i = is_sr? cnt1 - 1 : 1; i < cnt1; ++i) { // gap filling
		...
		if (i == cnt1 - 1 || (a[as1+i].y&MM_SEED_LONG_JOIN) || (qe - qs >= opt->min_ksw_len && re - rs >= opt->min_ksw_len)) {
			...
			if (opt->flag & MM_F_QSTRAND) {
				qseq = &qseq0[0][qs];
				rc_qseq = &qseq0[1][qlen-qs-1];
				mm_idx_getseq2(mi, rev, rid, rs, re, tseq);
			} else {
				qseq = &qseq0[rev][qs];
				rc_qseq = &qseq0[1-rev][qlen-qs-1];
				mm_idx_getseq(mi, rid, rs, re, tseq);
			}
			//fprintf(stderr, "\n qseq:%p: %d-%d; rc_qseq: %d-%d; rev: %d", qseq, qs, qe, qlen-qe-1, qlen-qs-1, rev);
			...
			//fprintf(stderr, "\n qseq:%p", qseq);
			if ((zdrop_code = mm_test_zdrop(km, opt, qseq, tseq, ez->n_cigar, ez->cigar, mat, rc_qseq, qlen)) != 0)
			...
			//fprintf(stderr, "\n\tzdrop return value for %p = %d; qs = %d, qe = %d, rs = %d, re = %d\n", qseq, zdrop_code, qs, qe, rs, re);
			// update CIGAR
			if (ez->n_cigar > 0)
				mm_append_cigar(r, ez->n_cigar, ez->cigar);
...
// Inserted the edit part of the code for converting to quantized seqs
#define SIZE 4096
...
{
	//fprintf(stderr, "\n\nEntered the kmervalue function with value %s", kmer);
...
void mm_kmermap_seq(int qlen, uint8_t *input_seq[2], int output_qlen, uint8_t *output_seq[2]) {
   ...
   FILE* ptr;
   char aline[20];
   ptr = fopen("/home/djjoshi/softwares/minimap2-2.24/quantizedmap.txt","r");
   int count = 0;
   //fprintf(stderr, "\n\nStarting the while loop: ptr = %p; aline = %s; err = %d", ptr, aline, errno);
   while(fscanf(ptr,"%s\n", aline)==1){
      //fprintf(stderr, "%s\n",aline);
      if(count >= 4){
         int akey;
         ...
         if (count%4 == 0){
			...
			//fprintf(stderr, "i = %d, akey = %d", count, akey);
   ...
   //fprintf(stderr, "\n\ninput_length %d; output_length %d", qlen, output_qlen);
   //fprintf(stderr, "\n\ninput_length %ld; output_length %ld", sizeof(*input_seq), sizeof(*output_seq));
...
mm_reg1_t *mm_align_skeleton(void *km, const mm_mapopt_t *opt, const mm_idx_t *mi, int qlen, const char *qstr, int *n_regs_, mm_reg1_t *regs, mm128_t *a, const char *qname)
{
	...
	const char **rc_qname;
	char *filename;
	filename = "/home/djjoshi/SV_call/simulations/expt_sim_sample_genome/nanosim_reads/reads.fasta";
	fprintf(stderr, "\nOpening file: %s\n", filename);
	rc_qname = (const char**)&filename;
	mm_bseq_file_t *fp;
	fp = (mm_bseq_file_t*)open_bseqs(1, rc_qname);
	if (fp == 0){
	...
	s->seq = mm_bseq_read(fp, 5000000000000000, 0, &s->n_seq);
	fprintf(stderr, "\nRead file successfully: %s\n", *rc_qname);
	for (int i = 0; i < s->n_seq; ++i){
		if (s->seq[i].name == qname){
			fprintf(stderr, "\nFound the name match in the file: %s\n", s->seq[i].name);
		}
	}
	mm_bseq_close(fp);
	*/
	...
	temp_seq[0] = (uint8_t*)malloc(qlen * 2);
	temp_seq[1] = temp_seq[0] + qlen;
	for (i = 0; i < qlen; ++i) {
		temp_seq[0][i] = seq_nt4_table[(uint8_t)qstr[i]];
		temp_seq[1][qlen - 1 - i] = temp_seq[0][i] < 4? 3 - temp_seq[0][i] : 4;
	}
	...
	mm_kmermap_seq(qlen+5, temp_seq, qlen, qseq0);*/
	...
	//char *filename;
	//filename = "/home/djjoshi/SV_call/simulations/expt_sim_sample_genome/nanosim_reads/rc_reads_q3.fasta";
	//gzFile fp;
	extern kseq_t *rc_seq;
	extern int line_count;
	//int l;
	char *rc_qstr;
	long int rc_length;
	extern char *rc_read_name[];
	extern char *rc_read_seq[];
	int index_i;
	
	//fp = gzopen(filename, "r");
	//seq = kseq_init(fp);
	for (i = 0; i < line_count; ++i){
		//fprintf(stderr, "\ni = %d/%d; qname: %s/%s", i, line_count, rc_read_name[i], qname);
		if (strcmp(rc_read_name[i],qname)==0){
			// fprintf(stderr, "\nname:%s; rc_length = %ld, qlen = %ld", qname, strlen(rc_read_seq[i]), qlen);
			// rc_qstr = (char*)malloc(sizeof(char)*(strlen(rc_read_seq[i])*2));
			// strcpy(rc_qstr, rc_read_seq[i]);
			// rc_qstr = rc_read_seq[i];
			// rc_length = strlen(rc_qstr);
			index_i = i;
			// fprintf(stderr, "\t loop finished:%d", index_i);
			break;
		}
	}

	//fprintf(stderr, "\nqname: %s: Length from file = %ld; qlen = %d; pointer: %p;\n", qname, rc_length, qlen, rc_qstr);
	for (i = 0; i < qlen; ++i) {
		//fprintf(stderr, "\nstarting: i=%d", i);
		qseq0[0][i] = seq_nt4_table[(uint8_t)qstr[i]];
		//fprintf(stderr, "\nMiddle: i=%d", i);
		//qseq0[1][qlen - 1 - i] = qseq0[0][i] < 4? 3 - qseq0[0][i] : 4;
		qseq0[1][i] = seq_nt4_table[(uint8_t)rc_read_seq[index_i][i]];
		//fprintf(stderr, "\tEnd: i=%d; qseq0: %d; qseq1: %d", i, qseq0[0][i], qseq0[1][i]);
	}
	//kseq_destroy(seq);
	//gzclose(fp);
